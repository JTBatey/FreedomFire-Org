...
- This folder IS the finished static website. Edit the top-level HTML files,
  `css/site.css`, `js/site.js`, and `assets/` directly.
...
- Run `python3 scripts/check_site.py` before handing off. It also checks that
  ...
  Update those regions together for site-wide changes. Never load them via
  fetch; direct-file browsing must work.
- Use explicit relative `.html` links. Main pages live at the root.
